...
namespace Z.PagedList.Web.Common
{
	public sealed class HtmlHelper
	{
		...
		{
			this._tagBuilderFactory = tagBuilderFactory;
		}
		...
		{
			var li = this._tagBuilderFactory
				.Create("li");
		...
		private ITagBuilder First(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options)
		{
			...
			var first = this._tagBuilderFactory
				.Create("a");
			...
			if (list.IsFirstPage)
			{
				return this.WrapInListItem(first, options, "PagedList-skipToFirst", "disabled");
			}

			first.Attributes.Add("href", generatePageUrl(targetPageNumber));

			return this.WrapInListItem(first, options, "PagedList-skipToFirst");
		}
		...
		private ITagBuilder Previous(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options)
		{
			...
			var previous = this._tagBuilderFactory
				.Create("a");
			...
			{
				return this.WrapInListItem(previous, options, options.PreviousElementClass, "disabled");
			}

			previous.Attributes.Add("href", generatePageUrl(targetPageNumber));

			return this.WrapInListItem(previous, options, options.PreviousElementClass);
		}
		...
		private ITagBuilder Next(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options)
		{
			...
			if (!list.HasNextPage)
			{
				return this.WrapInListItem(next, options, options.NextElementClass, "disabled");
			}

			next.Attributes.Add("href", generatePageUrl(targetPageNumber));

			return this.WrapInListItem(next, options, options.NextElementClass);
		}
		...
		private ITagBuilder Last(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options)
		{
			...
			var last = this._tagBuilderFactory
				.Create("a");
		...
		private ITagBuilder PreviousEllipsis(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options, int firstPageToDisplay)
		{
			...
			if (!list.HasPreviousPage)
			{
				return this.WrapInListItem(previous, options, options.EllipsesElementClass, "disabled");
			}
			...
			return this.WrapInListItem(previous, options, options.EllipsesElementClass);
		}
		...
		private ITagBuilder NextEllipsis(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options, int lastPageToDisplay)
		{
			...
			if (!list.HasNextPage)
			{
				return this.WrapInListItem(next, options, options.EllipsesElementClass, "disabled");
			}
			...
			return this.WrapInListItem(next, options, options.EllipsesElementClass);
		}
		...
		public string PagedListPager(IPagedList list, Func<int, string> generatePageUrl, PagedListRenderOptions options)
		{
			...
			{
				listItemLinks.Add(this.First(list, generatePageUrl, options));
			}
			...
			{
				listItemLinks.Add(this.Previous(list, generatePageUrl, options));
			}

			//text
			if (options.DisplayPageCountAndCurrentLocation)
			{
				listItemLinks.Add(this.PageCountAndLocationText(list, options));
			}

			//text
			if (options.DisplayItemSliceAndTotal)
			{
				listItemLinks.Add(this.ItemSliceAndTotalText(list, options));
			}
			...
			if (options.DisplayLinkToIndividualPages)
			{
				...
				{
					listItemLinks.Add(this.PreviousEllipsis(list, generatePageUrl, options, firstPageToDisplay));
				}
				...
				foreach (var i in Enumerable.Range(firstPageToDisplay, pageNumbersToDisplay))
				{
					...
					{
						listItemLinks.Add(this.WrapInListItem(options.DelimiterBetweenPageNumbers));
					}

					//show page number link
					listItemLinks.Add(this.Page(i, list, generatePageUrl, options));
				}
				...
				{
					listItemLinks.Add(this.NextEllipsis(list, generatePageUrl, options, lastPageToDisplay));
				}
			}
			...
			{
				listItemLinks.Add(this.Next(list, generatePageUrl, options));
			}
			...
			{
				listItemLinks.Add(this.Last(list, generatePageUrl, options));
			}
			...
			var ul = this._tagBuilderFactory
				.Create("ul");
			...
			var outerDiv = this._tagBuilderFactory
				.Create("div");
		...
		{
			var form = this._tagBuilderFactory
				.Create("form");
			...
			var fieldset = this._tagBuilderFactory
				.Create("fieldset");

			var label = this._tagBuilderFactory
				.Create("label");
			...
			var input = this._tagBuilderFactory
				.Create("input");
			...
			var submit = this._tagBuilderFactory
				.Create("input");
