...
namespace Z.PagedList
{
	public class PagedList<T, TKey> : BasePagedList<T>
	{
			...
			: base(pageNumber, pageSize, superset?.Count() ?? 0)
		{
			...
			{
				this.InitSubset(superset, keySelector.Compile(), pageNumber, pageSize);
			}
		}
			...
			: base(pageNumber, pageSize, superset?.Count() ?? 0)
		{
			if (this.TotalItemCount > 0)
			{
				this.InitSubset(superset, keySelectorMethod, pageNumber, pageSize);
			}
		}
